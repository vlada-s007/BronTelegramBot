...
from bronTelegramBot.utils import timedelta_to_datetime


async def booking_menu_markup():
    builder = InlineKeyboardBuilder()
    builder.button(text=f'🗂️ ' + _('Choose Services By Category'), callback_data='categoryChoose')
    builder.button(text=f'🔎 ' + _('Search Services and Products'), callback_data='searchBooking')
...
async def branch_choices(state: dict, *args):
    ...
    if state.get('category'):
        ...
        page_buttons.button(text='⬅️🔎 ' + _('Return to Search Results'),
...
async def choose_alone_or_with_guests(date):
    builder = InlineKeyboardBuilder()
    builder.button(text='👤' + _("I'm making a reservation for myself only"))
    builder.button(text='👥' + _("I'm coming with guests (proceed to choosing the amount of guests)"))
    page_buttons = InlineKeyboardBuilder()
    page_buttons.button(text=f'⬅️🕔 ' + _('Return to choosing Hours'), callback_data=f'bookingDate_{date.strftime("%Y-%m-%d")}')
    builder.adjust(1, 1, 1)
    return builder.attach(page_buttons).as_markup()
...
async def note_buttons(staff_id):
    builder = InlineKeyboardBuilder()
    builder.button(text='⏩ ' + _('Skip this step'), callback_data='skipNoteStep')
    builder.button(text=f'⬅️👤 ' + _('Return choosing the number of guests'), callback_data=f'chooseStaff_{staff_id}')
    builder.adjust(1, 1)
    return builder.as_markup()
