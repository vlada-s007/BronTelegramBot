...
from BronBot.middlewares.database import search_user_by_tg_id, search_user_by_phone_number, update_database_tg_id, \
...
class AuthMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if event.contact and not state_data.get('auth') and not user_database_id:
            ...
            phone_in_database = await search_user_by_phone_number(phone)
            print(phone_in_database)
            await state.clear()
            await state.update_data(user_id=phone_in_database)
            await state.update_data(notifications=True)
            await state.update_data(chat_id=event.chat.id)
            # only for local database, remove when production is over
